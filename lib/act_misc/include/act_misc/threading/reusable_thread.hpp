 ...
 *       start.
 *       Therefore, this class can be costly if the thread function is called frequently. Prefer it
 *       if the task can only be started occasionally and if only one instance of the task should
 *       run at a time.
 */
class ReusableThread
{
  ...
  public:
     ...
     * @param waitToJoin Whether to wait for the previous thread to join if it's running. If false,
     *                   the method returns ALREADY_RUNNING.
     * @param logger The logger to use for logging
     * @param preconditionFunc The function that returns a value to send to the thread function.
     *                         If the function returns a nullopt, this means that an error occurred
     *                         and the thread will not be started.
     *                         The value returned by the precondition function is passed as the
     * first argument to the thread function, before the other arguments.
     ...
     */
    template <typename T, typename Func, typename... Args>
    ReusableThreadResult::Enum start(const AbsLogger &logger,
                                     const std::function<std::optional<T>()> &preconditionFunc,
                                     Func &&func,
                                     Args &&...args);
     ...
     */
    template <typename Func, typename... Args>
    ReusableThreadResult::Enum start(const AbsLogger &logger, Func &&func, Args &&...args);
...
template <typename Func, typename... Args>
inline ReusableThreadResult::Enum ReusableThread::start(const AbsLogger &logger,
                                                        Func &&func,
                                                        Args &&...args)
{
