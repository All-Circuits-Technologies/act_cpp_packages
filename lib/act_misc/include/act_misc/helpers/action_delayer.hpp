...
class ActionDelayer
{
  public:
     ...
     *                   executed even if the delay is restarted multiple times. If not provided,
     *                   there is no maximum delay and the action will be executed only after the
     *                   delay expires without being restarted.
     * @param startImmediately Flag indicating whether the delay should start immediately upon
     *                         construction
     */
    explicit ActionDelayer(const AbsLogger &logger,
  ...
  public:
    ...
    /**
     * @brief Start the delay if it is not already running. If the delay is already running, it will
     *        be restarted, but not the maximum delay timer if it was provided.
     */
    void startOrDelay();
  ...
  private:
     ...
     * @brief Start or restart the delay. If the delay is already running, it will be restarted,
     *        same for the maximum delay if it was provided.
     * @note This method isn't protected by the mutex, it is the caller responsibility to call it in
     *       a thread-safe way.
     */
    void startOrRestartProcess();

    /**
     * @brief Internal callback for the delay timer. This is the callback that will be called by the
     *        delay timer when the delay expires. It will call the user-provided callback and
     *        stop the delay.
